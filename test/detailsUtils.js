...
//        currentDishPromiseState:{ },
    ...
    it("Details presenter 'no data' when current dish is not set",  async function tw_3_2_50_2(){
        ...
        removed=0;
        observers=[];
        modelTarget.currentDishId=null;
        await doRender();
...
function checkModelFetch(){
    it("DinnerModel setCurrentDishId does not initiate a fetch", function tw_3_2_70_5(){
        try{
            modelTemplate.setCurrentDishId(314);
        }catch(e){
            if(e.message.indexOf("made a fetch that is not expected by the tests")!=-1)
                expect.fail("The model initiated a fetch in setCurrentDishId:\n"+e);
            throw e;
        }
    });
